...
const live2d_path_local = constant.live2d_path_local; // 新增本地资源路径

// 判断是否加载本地资源
const useLocal = constant.useLocal || false;

// 加载 waifu.css live2d.min.js
if (screen.width > 768) {
  Promise.all([
    loadExternalResource(live2d_path + "waifu.css", "css").catch(() => loadExternalResource(live2d_path_local + "waifu.css", "css")),
    loadExternalResource(live2d_path + "live2d.min.js", "js").catch(() => loadExternalResource(live2d_path_local + "live2d.min.js", "js"))
  ]).then(() => {
    initWidget({
      waifuPath: constant.baseURL + constant.waifuPath,
      cdnPath: useLocal ? live2d_path_local : constant.cdnPath
    });
  }).catch((error) => {
